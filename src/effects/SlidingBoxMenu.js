...
const SlidingBoxMenu = () => {
    ...
    const effectRenderContainer = document.getElementById("effect-render-container");
    
    effectRenderContainer.innerHTML = '';
    
    slidingDemoNav.render(effectRenderContainer);
    const startingSect = location.hash.length > 1 ? location.hash.slice(1) : 'home';
    slidingDemoNav.toggleActive(startingSect);
    sections.render(effectRenderContainer);
